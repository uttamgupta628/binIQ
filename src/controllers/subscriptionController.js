...
const verifyStoreOwnerSubscription = async (req, res) => {
  try {
    ...
    const user = await User.findById(storeOwnerId).select(
      "role verified status subscription subscription_end_time",
    );
    ...
    const isVerified =
      user.verified === true && user.status === "approved" && notExpired;
